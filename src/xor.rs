...
pub fn xor_cypher_decrypt_char_frequency(s: &String) -> String {
    ...
    let mut min_score: f32 = f32::INFINITY;
    ...
    for key in 0..255 as u8 {
        ...
        let utf8_string;
        match str::from_utf8(&cleartext_candidate) {
            Err(_) => {
                continue;
            }
            Ok(s) => {
        ...
        let score = score_candidate(&utf8_string);
        if score < min_score {
            min_score = score;
            best_candidate = utf8_string;
        }
    }
...
fn score_candidate(s: &String) -> f32 {
    ...
    let frequency_data: [(char, f32); 26] = [
        ('a', 0.0812),
        ('b', 0.0149),
        ('c', 0.0271),
        ('d', 0.0432),
    ...
    let mut score = 0.0;
    for &(c, freq) in frequency_data.iter() {
        score += ((*map.get(&c).unwrap_or(&0) as f32 / s.len() as f32) - freq)
            .abs()
            .ln();
    }
